...
function imprimeuf($ufsec){
    ...
    function validaCPF($cpf) {
      ...
      // $data['erro'] = ['required','isnumeric','min'=>10,'max'=>20]]
            ...
            case 'isstring':
              if($required || !empty($data)){
                ...
                if(!empty($option['min']) && (strlen($data) < $option['min'])){
                    return 'Texto tem que ter no mínimo ' . $option['min'] . ' caracteres!';
                }
                if(!empty($option['max']) && (strlen($data) > $option['max'])){
                  return 'O texto excede o máximo de ' . $option['max'] . ' caracteres!';
                }                 
              }              
              break;
            /* $data['erro'] = ['isnumeric','min'=>10,'max'=>20]] número mínimo permitido e número máximo */
            case 'isnumeric':             
              if($required || !empty($data)){
                if(!is_numeric($data)){
